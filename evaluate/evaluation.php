...
<script src="js/evaluation.js"></script>
	<?php
		...
		if(isset($_SESSION['login']))
		{
			...
			{
				$_SESSION['redirectspe'] = $_GET['spe'];
				header("Location:../login/norightredirect.php");
			}
		...
		{
			$_SESSION['redirectspe'] = $_GET['spe'];
			$_SESSION['redirectucode'] = $_GET['unit'];
			header("Location:../login/logintoaccessredirect.php");
		}
		...
		/*Check whether submitted, if so redirect to cannot fill in multiple*/
		$sql_checkcompletion = "SELECT * FROM spe_completionstatus WHERE StudentID = '$student_id' AND SPEID = '$speid'";
		$result_checkcompletion = $connection->query($sql_checkcompletion);
		if ($result_checkcompletion->num_rows > 0){
			header("Location:submitted.php");
		}
		
		$sql_checkcanfill = "SELECT * FROM studentlistdetail WHERE StudentID = '$student_id' AND UnitCode = '$unit_code'";
		$result_checkcanfill = $connection->query($sql_checkcanfill);
		if ($result_checkcanfill->num_rows < 1){
		...
		if (isset($_SESSION["RecordCounter"]))
		{
			...
			{	
				//echo "<script>alert('record counter if not equal to 0 ".$_SESSION["RecordCounter"]."');</script>";
			...
			else if(($_SESSION["RecordCounter"]) == 0)
			{
				//echo "<script>alert('record counter if equal to 0 ".$_SESSION["RecordCounter"]."');</script>";
				$offset_counter = 0;
