<?php
	...
	$error;
	if(isset($_SESSION['redirectspe']))
	{
		$speid = $_SESSION['redirectspe'];
	}
	else
	{
	...
	if(isset($_SESSION['redirectucode']))
	{
		$unitid = $_SESSION['redirectucode'];
	}
	else
	{
		header("Location:noright.php");
	...
	if(isset($_POST['submit']))
	{
		...
		if(isset($pieces[2]))
		{
			$role = explode("+",$pieces[2]);
		}
		else
		{
			$role = explode("+","no+record");
		}
		
		if ($role[0] == "student") //Look through in student db
		{
			$sql_student = "SELECT * FROM login WHERE Email = '$userid' AND Password = '$password'";
			$result_student = $connection -> query($sql_student);
			if ($result_student -> num_rows == 1){
				while($row_stu = $result_student->fetch_assoc()) {
					$_SESSION['studentid'] = $row_stu['ID'];
					$_SESSION['login'] = 1;
					header("Location:../evaluate/evaluation.php?spe=$speid&unit=$unitid");
				}
			}
			else
			{
		...
		else if ($role[0] == "uc")//Look through uc db
		{
			$sql_student = "SELECT * FROM login WHERE Email = '$userid' AND Password = '$password'";
			$result_student = $connection -> query($sql_student);
			if ($result_student -> num_rows == 1){
...
<div class="col-md-12" style="padding:0px;">
	<h3 class="col-md-12" onclick="window.location.href='../login/index.php'" style="cursor:pointer; padding-top:8px;padding-bottom:8px;text-align:left;font-size:x-large;font-weight:bold;background-color:#1d1a1a;color:#ffffff;"><img style="cursor:pointer;width:100px;height:30px;margin-right: 15px;" src="images/murdoch_logo.png">Self Peer Evaluation System</h3>
