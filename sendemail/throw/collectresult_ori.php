<?php
	...
	include "../zphpspreadsheet/vendor/autoload.php";
	...
	$today = date("Y-m-d") ;
	$due =date('Y-m-d', strtotime($today. ' - 1 days')); 
	$sql_selectduespe = "SELECT * from spe WHERE DueDate = '$due' AND Visibility = TRUE";
	$request_duespe= $connection->query($sql_selectduespe);
	if ($request_duespe-> num_rows>0){
		while ($row_duespe= $request_duespe->fetch_assoc())
		{ //Each record that is due
				...
				while ($row_group= $request_group->fetch_assoc()){//Starthere for each group record
					...
					if ($request_name-> num_rows>0)
					{
						...
						{ 
							...
							array_push($finalrecordarray,$finalscore);
			...
			if ($request_uc-> num_rows>0){
				...
				$sql_selectmemberinsert = "SELECT * from studentlistdetail WHERE UnitCode = '$unitcode' AND TeachPeriod = '$period'";
				$request_memberinsert= $connection->query($sql_selectmemberinsert);
				if ($request_memberinsert-> num_rows>0)
				{
					while ($row_memberinsert= $request_memberinsert->fetch_assoc())
					{
						$studentidtosave = $row_memberinsert['StudentId'];
						$studentteamid = $_row_memberinsert['TeamId'];
						//Check whether completed
						$sql_selectmembercomplete = "SELECT * from spe_completionstatus WHERE SPEID = '$speid' AND StudentID = '$studentidtosave'"; //select spe that due
						$request_membercomplete= $connection->query($sql_selectmembercomplete);
						if ($request_membercomplete-> num_rows>0) //if got record calculate score
						{
							//Select each member from the same team
							$sql_selectallmember = "SELECT * from studentlistdetail WHERE UnitCode = '$unitcode' AND TeachPeriod = '$period' AND TeamId = '$studentteamid'";
							$request_allmember= $connection->query($sql_selectallmember);
							if ($request_allmember-> num_rows>0)
							{
								while ($row_allmember= $request_allmember->fetch_assoc())
								{
									$allmemberid = $row_allmember['StudentId'];
									//Check whether fill in, if yes calculate average, if no put 0
									$sql_allmemberspe = "SELECT * from spe_completionstatus WHERE SPEID = '$speid' AND StudentID = '$allmemberid'"; //select spe that due
									$request_allmemberspe= $connection->query($sql_allmemberspe);
									if ($request_allmemberspe-> num_rows>0) //if got record calculate score
									{
										$completionid = $request_allmemberspe['CompletionID'];
										$sql_selectmemberscore = "SELECT AVG(Score) As AverageScore FROM spe_result LEFT JOIN spe_completionstatus
														ON spe_result.CompletionID = spe_completionstatus.CompletionID
														WHERE CompletionID = '$completionid'
														GROUP BY '$studentidtosave'";
										$request_memberscore= $connection->query($sql_selectmemberscore);
										if ($request_memberscore-> num_rows>0)
										{
											while ($row_memberscore= $request_memberscore->fetch_assoc())
											{
												$averagescore = $row_memberscore['AverageScore'];
												$totalaverage = $totalaverage + $averagescore;
												$numberofrecord++;
											}
										}
									}
							...
							$finalscoreavg = $totalaverage/$numberofrecord;
							$sql_insert_finalscore= " INSERT INTO spe_finalscore (StudentId,SPEID,Score) VALUES('$studentidtosave', '$speid','$finalscoreavg') ";
							if($connection->query($sql_insert_$sql_insert_finalscore) === TRUE){
							...
							//$completionid = $request_membercomplete['CompletionID'];
							/*$sql_selectmemberscore = "SELECT SUM(Score) from spe_result LEFT JOIN spe_completionstatus
														ON spe_result.CompletionID = spe_completionstatus.CompletionID
														WHERE UnitCode = '$unitcode' AND TeachPeriod = '$period'
														GROUP BY spe_result.CompletionID
														HAVING ";
							$request_memberscore= $connection->query($sql_selectmemberscore);
							if ($request_memberscore-> num_rows>0)
							{
						...
						{
							$sql_insert_finalscore= " INSERT INTO spe_finalscore (StudentId,SPEID,Score) VALUES('$studentidtosave', '$speid',0) ";
							if($connection->query($sql_insert_$sql_insert_finalscore) === TRUE){
								//echo "Student list detail insert Successful";
								//echo "<br>";
							}else{
					...
					/*$number = 0;
					/while ($row_member= $request_memberinsert->fetch_assoc())//this will be each student in the group
					{
						$finalmark = $finalrecordarray[$number];
						$sql_insert_studentlistdetail = " INSERT INTO spe_finalscore (StudentId,SPEID,Score) VALUES('$receiverstuid', '$speid','$finalmark') ";
