			...
			//Insert final score to spe_finalscore table
				...
				if ($request_memberinsert-> num_rows>0)
				{
					while ($row_memberinsert= $request_memberinsert->fetch_assoc())
					{
						...
						$studentteamid = $row_memberinsert['TeamId'];
						...
						{
							/*$sql_insert_finalscore= " INSERT INTO spe_finalscore (StudentId,SPEID,Score) VALUES('$studentidtosave', '$speid',1) ";
							if($connection->query($sql_insert_finalscore) === TRUE){
								//echo "Student list detail insert Successful";
								//echo "<br>";
							}else{
								//echo "Student list detail insert Failed";
							...
							if ($request_allmember-> num_rows>0)
							{
								...
								while ($row_allmember= $request_allmember->fetch_assoc())
								{
									...
									//echo "The member id is ".$allmemberid."              ";
									...
									{
										$eachaverage = 0;
										while ($row_allmemberspe= $request_allmemberspe->fetch_assoc())
										{
											$completionid = $row_allmemberspe['CompletionID'];
											/*$sql_selectmemberscore = "SELECT * FROM spe_result LEFT JOIN spe_completionstatus
															ON spe_result.CompletionID = spe_completionstatus.CompletionID
															GROUP BY '$studentidtosave'";
															//HAVING spe_result.CompletionID = $completionid";*/
											$sql_selectmemberscore = "SELECT * FROM spe_result WHERE CompletionID = '$completionid' AND ReceiverStudentID = '$studentidtosave'";		
											...
											if ($request_memberscore-> num_rows>0)
											{
												...
												{
													$averagescore = $row_memberscore['Score'];
												...
												$eachaverage = $totalaverage/$numberofrecord;
											...
											$numberofcompletedstudent++;
										}
										$finalavg = $eachaverage + $finalavg;
										$numberofcompletedstudent;
									}
									else
									{
								...
								$finalscoreavg = ($finalavg/$numberofcompletedstudent)/2;
							}
							/*if ($numberofrecord == 0)
							{
								$finalscoreavg = 0;
