<?php
	...
	$sql_selectduespe = "SELECT * from spe WHERE DueDate = '$sevendays' AND Visibility = TRUE";
		...
		while ($row_duespe= $request_duespe->fetch_assoc()) //select record that has not submitted 7days before submission date
		{
			...
			if ($request_student-> num_rows>0)
			{
				while ($row_student= $request_student->fetch_assoc())
				{
					...
					{
						$to = $email;
						$subject = $unitcode."-Reminder: Self Peer Evaluation Due in 7 days";
						$message = "
						Hi ".$fullname.",
						
						Please note that Self Peer Evaluation for ".$unitcode." is going to due in 7 days.

						Please complete before the due date:
						http://ict302-tma-ft07.ad.murdoch.edu.au/evaluate/evaluation.php?spe=$speid&unit=$unitcode
