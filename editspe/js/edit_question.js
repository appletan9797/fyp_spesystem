$(document).ready(function(){
		...
		else
		{
			if(row_num <= max_rows)
			{
				$("#container").append("<div class='cQuestionContainer'>"
							...
							+ "<option value='' class='option' id='option' selected disabled>Section</option>"
	...
	$("body").on('click', '.save_button', function(){
		if (jsonString != null)
		{
			...
			row_num = 1;
			//alert("save");
			if ($('div.cQuestionContainer').length == 0)
			{
				//$("#cannotdelete").html("You must have at least one question in your SPE");
				$.ajax
				({
					type:'POST', //the way to pass data
					url: 'get_question_ajax.php', //where to pass, which contains sql
					data:
					{
						'id' :$speid, //data to pass to the file (url)
						'ucode': ucodefromphp,
					},
					success: function(data) //on success do something
					{
